	...
	public class ConvexHull
	{
		...
		public void CalcConvexHull(ConvexHullThreadUsage threadUsage = ConvexHullThreadUsage.OneOrFour)
		{
			...
			if (threadUsage == ConvexHullThreadUsage.OnlyOne)
			{
				...
				_q1.PrepareQuadrant(true);
				_q2.PrepareQuadrant(true);
				_q3.PrepareQuadrant(true);
				_q4.PrepareQuadrant(true);
			}
			else
			{
				...
				for (int n = 0; n < tasks.Length; n++)
				{
					...
					tasks[n] = Task.Factory.StartNew(() => quadrants[nLocal].PrepareQuadrant(isSkipSetQuadrantLimit));
				}
				Task.WaitAll(tasks);
			}
		}
		...
		public Point[] GetResultsAsArrayOfPoint()
		{
			...
			indexQ1Start = 0;
			indexQ1End = _q1.HullPointCount - 1;
			lastPoint = _q1.HullPoints[indexQ1End];

			if (_q2.HullPointCount == 1)
			{
			...
			else
			{
				...
				indexQ2End = _q2.HullPointCount - 1;
				lastPoint = _q2.HullPoints[indexQ2End];
			}

			if (_q3.HullPointCount == 1)
			{
			...
			else
			{
				...
				indexQ3End = _q3.HullPointCount - 1;
				lastPoint = _q3.HullPoints[indexQ3End];
			}

			if (_q4.HullPointCount == 1)
			{
			...
			else
			{
				...
				indexQ4End = _q4.HullPointCount - 1;
